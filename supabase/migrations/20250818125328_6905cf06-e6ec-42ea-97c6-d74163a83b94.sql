...
DROP VIEW IF EXISTS public.company_members_v;

-- Create profiles table (will be no-op if exists)
...
-- Create companies table (will be no-op if exists) 
...
-- Drop and recreate user_company table to ensure correct schema
DROP TABLE IF EXISTS public.user_company CASCADE;
...
-- Now create the view with correct column references
CREATE VIEW public.company_members_v AS
SELECT 
  uc.company_id, 
  uc.role, 
  uc.status, 
