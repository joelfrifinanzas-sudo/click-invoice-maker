...
SELECT 
  ...
  email,
  company_id,
  COALESCE(created_by, (SELECT owner_user_id FROM companies WHERE id = clientes.company_id LIMIT 1)) as created_by,
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'customers' AND table_schema = 'public') THEN
    ...
    SELECT 
      ...
      email,
      company_id,
      COALESCE(owner_user_id, (SELECT owner_user_id FROM companies WHERE id = customers.company_id LIMIT 1)) as created_by,
      'active' as status,
      false as archived,
      created_at
