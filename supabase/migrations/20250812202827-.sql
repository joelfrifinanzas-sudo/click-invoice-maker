-- Company-scoped roles via company_members with reusable RLS helpers
-- Scope: Create table + RLS + helper functions + apply policies to company_id tables
...
create policy if not exists "cmembers_select_same_company"
  on public.company_members
  for select
  using (public.is_company_member(company_id));

-- Insert: only SUPER_ADMIN/ADMIN of that company
create policy if not exists "cmembers_admin_insert"
  on public.company_members
  for insert
  with check (public.is_company_admin(company_id));

-- Update: only SUPER_ADMIN/ADMIN of that company
create policy if not exists "cmembers_admin_update"
  on public.company_members
  for update
...
create policy if not exists "cmembers_admin_delete"
  on public.company_members
  for delete
  using (public.is_company_admin(company_id));

-- 4) Apply reusable condition to app tables with company_id
-- NOTE: We add policies without removing existing ones to avoid breaking current access; these can be tightened later.

-- products: members can read; admins can write
create policy if not exists "cm_read_products"
  on public.products
  for select
  using (public.is_company_member(company_id));

create policy if not exists "cm_insert_products_admin_only"
  on public.products
  for insert
  with check (public.is_company_admin(company_id));

create policy if not exists "cm_update_products_admin_only"
  on public.products
  for update
  using (public.is_company_admin(company_id))
  with check (public.is_company_admin(company_id));

create policy if not exists "cm_delete_products_admin_only"
  on public.products
  for delete
  using (public.is_company_admin(company_id));

-- invoices: members can read
create policy if not exists "cm_read_invoices"
  on public.invoices
  for select
  using (public.is_company_member(company_id));

-- cotizaciones: members can read
create policy if not exists "cm_read_cotizaciones"
  on public.cotizaciones
  for select
  using (public.is_company_member(company_id));

-- empresa_cotizacion_config: members can read
create policy if not exists "cm_read_empresa_cotizacion_config"
  on public.empresa_cotizacion_config
  for select
  using (public.is_company_member(company_id));

-- empresa_facturacion_config: members can read
create policy if not exists "cm_read_empresa_facturacion_config"
  on public.empresa_facturacion_config
  for select
  using (public.is_company_member(company_id));

-- empresa_ncf_sequences: members can read
create policy if not exists "cm_read_empresa_ncf_sequences"
  on public.empresa_ncf_sequences
  for select
  using (public.is_company_member(company_id));

-- customers: members can read
create policy if not exists "cm_read_customers"
  on public.customers
  for select
  using (public.is_company_member(company_id));

-- audit_events: members can read their company's events (additional to existing)
create policy if not exists "cm_read_audit_events"
