...
export const InvoiceActions = ({ invoiceData, invoiceNumber, onBack }: InvoiceActionsProps) => {
  ...
  const baseButton = "touch-target h-12 min-h-[44px] rounded-xl border border-border bg-card text-foreground hover:bg-accent hover:text-accent-foreground shadow-sm flex items-center justify-center gap-2 px-4";
  const handlePrint = () => {
    window.print();
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mb-4 print:hidden">
      {isMobile ? (
        <div className="flex gap-3 overflow-x-auto -mx-4 px-4 py-1">
          <Button
            variant="outline"
            onClick={onBack}
            className={`${baseButton} shrink-0 min-w-[180px]`}
          >
          ...
          <Button
            ...
            disabled={isSharing}
            aria-busy={isSharing}
            className={`${baseButton} shrink-0 min-w-[220px]`}
          >
            ...
            <span className="text-sm font-semibold">{isSharing ? 'Enviando…' : 'Enviar por WhatsApp'}</span>
          </Button>
          ...
          <Button
            ...
            className={`${baseButton} shrink-0 min-w-[200px]`}
          >
            <Download className="w-5 h-5" />
            <span className="text-sm">{isGeneratingPDF ? 'Generando PDF...' : 'Descargar PDF'}</span>
          </Button>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="lg:col-span-3">
              ...
              >
                ...
                <span className="text-lg font-semibold">{isSharing ? 'Enviando…' : 'Enviar por WhatsApp'}</span>
              </Button>
            </div>
